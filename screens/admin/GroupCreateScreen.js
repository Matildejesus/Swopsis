...
function GroupCreateScreen() {
  ...
  const submitHandler = async () => {
    ...
    if (!name || !description || !location || !rules || !avatar || !ambassadorEmail) {
...
console.log(ambassadorEmail);
      ...
      const groupData = await addGroup({
        group: {
          name, description, location, rules, numberOfMem: 1, avatar, ambassador: ambassador.id
        }
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        <PrimaryButton title={"CREATE"} style={styles.button} onPress={submitHandler}/>
      </View>
    </View>
  );
};
